...
var S = {
  init: function () {
    ...
    if (i !== -1) {
      ...
      S.UI.simulate('Hi Baby|生日快乐|Happy Birthday|#countdown 3||');
    }
    ...
    S.Drawing.loop(function () {
      ...
      if(m<700){
        ...
        if(angle < 30){
        S.Drawing.drawHeart(angle, heart);
        angle += 0.2;
        sleep(1000 / 100);
        }
        if(angle>30 && time==20){start=(new Date()).getTime();}
        if(angle>30 && time<=20) {
        time = (start - (new Date()).getTime())/1000+20;
        document.querySelector('.bt2').innerHTML = "<span style='color:rgb(255, 255, 255);font-size:20px'>"+Math.floor(time)+"</span>";
        if(time <= 0){
...
S.Drawing = (function () {
  ...
  return {
    ...
    adjustOffsetXY: function () {
      offsetX = $(window).width() / 2;
      offsetY = $(window).height() / 2 - 20;
    },
    ...
    drawHeart: function(angle, heart) {
        var bloom = S.Drawing.getHeartPoint(angle);
		var draw = true;
